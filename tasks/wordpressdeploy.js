...
module.exports = function(grunt) {
  ...
  grunt.registerTask("push_db", "Push to Database", function() {
    ...
    if (
      ...
      typeof grunt.config.get("wordpressdeploy")[target] === "undefined"
    ) {
      grunt.fail.warn(
        "Invalid target specified. Did you pass the wrong argument? Please check your task configuration.",
        6
      );
    }
    ...
    grunt.log.subhead(
      "Pushing database from 'Local' to '" + target_options.title + "'"
    );
  ...
  grunt.registerTask("pull_db", "Pull from Database", function() {
    ...
    if (
      ...
      grunt.fail.warn(
        "Invalid target provided. I cannot pull a database from nowhere! Please checked your configuration and provide a valid target.",
        6
      );
    }
    ...
    // Start execution
    grunt.log.subhead(
      "Pulling database from '" + target_options.title + "' into Local"
    );
  ...
  grunt.registerTask(
    "push_files",
    "Transfer files to a remote host with rsync.",
    function() {
      var task_options = grunt.config.get("wordpressdeploy")["options"];
      var target = grunt.option("target") || task_options["target"];
      ...
      if (
        ...
        grunt.fail.warn(
          "Invalid target provided. I cannot push files from nowhere! Please checked your configuration and provide a valid target.",
          6
        );
      }
      ...
      var fromPath = local_options.path + "/wp-content/uploads/";
      var toPath = target_options.path + "/wp-content/uploads/";
      var config = {
  ...
  grunt.registerTask(
    ...
    function() {
      ...
      var fromPath = target_options.path + "/wp-content/uploads/";
      var toPath = local_options.path + "/wp-content/uploads/";
